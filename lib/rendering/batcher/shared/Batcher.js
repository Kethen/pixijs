...
var maxRecommendedTextures = require('../../renderers/shared/texture/utils/maxRecommendedTextures.js');
...
const _Batcher = class _Batcher {
  constructor(options = {}) {
    ...
    options = __spreadValues(__spreadValues({}, _Batcher.defaultOptions), options);
    const { vertexSize, indexSize } = options;
    this.attributeBuffer = new ViewableBuffer.ViewableBuffer(vertexSize * this._vertexSize * 4);
  ...
  break(instructionSet) {
    ...
    let textureBatch = this._textureBatchPool[this._textureBatchPoolIndex++] || new BatchTextureArray.BatchTextureArray();
    textureBatch.clear();
    ...
    for (let i = this.elementStart; i < this.elementSize; ++i) {
      ...
      if (textureBatch.count >= maxTextures || blendModeChange) {
        ...
        textureBatch = this._textureBatchPool[this._textureBatchPoolIndex++] || new BatchTextureArray.BatchTextureArray();
        textureBatch.clear();
